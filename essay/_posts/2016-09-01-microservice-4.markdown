...
在实现上，原有系统实现的类结构图如下：  
[![原有架构](http://static.cocolian.org/img/in-post/gateway-legacy.jpg)](http://static.cocolian.org/img/in-post/gateway-legacy.jpg)  
...
[![原有架构-渠道](http://static.cocolian.org/img/in-post/gateway-channels.jpg)](http://static.cocolian.org/img/in-post/gateway-channels.jpg)  
...
[![链式模式](http://static.cocolian.org/img/in-post/pattern-chain.png)](http://static.cocolian.org/img/in-post/pattern-chain.png)  
...
[![链式模式](http://static.cocolian.org/img/in-post/gateway-chain.jpg)](http://static.cocolian.org/img/in-post/gateway-chain.jpg)

## 网关拆分(Proxy Pattern)

如上所述，支付网关按照使用场景进行拆分。我们采用完善一个、接入一个的原则，在保留旧网关的功能的同时，开发完善新的网关和支付产品。等所有流量都打到新网关上去之后，旧网关就直接废弃了。为了达到这个目标，我们引入了代理模式：

[![代理模式](http://static.cocolian.org/img/in-post/pattern-proxy.png)](http://static.cocolian.org/img/in-post/pattern-proxy.png)  
...
[![代理模式](http://static.cocolian.org/img/in-post/gateway-proxy.jpg)](http://static.cocolian.org/img/in-post/gateway-proxy.jpg)
...
[![聚合模式](http://static.cocolian.org/img/in-post/pattern-aggregator.png)](http://static.cocolian.org/img/in-post/pattern-aggregator.png)  
...
[![聚合模式](http://static.cocolian.org/img/in-post/gateway-aggregator.jpg)](http://static.cocolian.org/img/in-post/gateway-aggregator.jpg)  

在支付产品的实现流程中，首先需要对参数进行校验，校验成功后，调用风控检查该交易是否可以放行。这两个操作，在处理上可以并行，使用的是分支模式。 
[![分支模式](http://static.cocolian.org/img/in-post/pattern-branch.png)](http://static.cocolian.org/img/in-post/pattern-branch.png)  

> 分支模式是聚合模式的扩展，可以允许同时调用两个或者更多的微服务。 

[![分支模式](http://static.cocolian.org/img/in-post/gateway-branch.jpg)](http://static.cocolian.org/img/in-post/gateway-branch.png)  
...
[![聚合模式](http://static.cocolian.org/img/in-post/gateway-agg-channels.jpg)](http://static.cocolian.org/img/in-post/gateway-agg-channels.jpg)  

## 通知商户 (Asynchronous Messaging Pattern)

支付产品执行的最后一个步骤是通知调用方支付的结果。 原系统实现是将这个步骤耦合在原有代码中，容易受到调用方接口的稳定性的影响。 为此，这里采用异步消息的模式来进行重构：

[![异步消息模式](http://static.cocolian.org/img/in-post/pattern-messaging.png)](http://static.cocolian.org/img/in-post/pattern-messaging.png)  
...
[![异步模式](http://static.cocolian.org/img/in-post/gateway-messaging.jpg)](http://static.cocolian.org/img/in-post/gateway-messaging.jpg)  
